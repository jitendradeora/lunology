...
import { Menu, X, ChevronDown, Moon, Sun, Globe, Instagram, Twitter, Facebook } from 'lucide-react';
...
export function Navigation() {
  ...
  const digitalProducts = [
    { id: 8, name: 'Digital Dream Interpretation Guide', nameAr: 'الدليل الرقمي لتفسير الأحلام', image: 'https://cdn.salla.sa/dPjdmy/CCRZObFc4mPA7MaIYvrrPstzCEn7RKhu78oaWCxr.png' },
    { id: 9, name: 'Digital Lunar Calendar 2026', nameAr: 'التقويم القمري الرقمي 2026', image: 'https://cdn.salla.sa/dPjdmy/CCRZObFc4mPA7MaIYvrrPstzCEn7RKhu78oaWCxr.png' },
    { id: 10, name: 'Meditation & Intention Workbook', nameAr: 'كتاب عمل التأمل وتحديد النية', image: 'https://cdn.salla.sa/dPjdmy/CCRZObFc4mPA7MaIYvrrPstzCEn7RKhu78oaWCxr.png' },
  ];

  const printedProducts = [
    { id: 5, name: 'Dreamology Journal 2026', nameAr: 'مفكرة دريمولوجي 2026', image: 'https://cdn.salla.sa/dPjdmy/CCRZObFc4mPA7MaIYvrrPstzCEn7RKhu78oaWCxr.png' },
    { id: 6, name: 'Annual Planner 2026', nameAr: 'المفكرة السنوية 2026', image: 'https://cdn.salla.sa/dPjdmy/CCRZObFc4mPA7MaIYvrrPstzCEn7RKhu78oaWCxr.png' },
    { id: 7, name: 'Mindfulness Journal', nameAr: 'مفكرة اليقظة الذهنية', image: 'https://cdn.salla.sa/dPjdmy/CCRZObFc4mPA7MaIYvrrPstzCEn7RKhu78oaWCxr.png' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="relative group flex items-center">
            <img src={language === 'ar' ? logoLunologyAr : logoLunologyEn} alt="Lunology" className="h-12 w-auto" />
          </Link>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link to="/elements" className="relative group text-sm tracking-wide">
              {language === 'ar' ? 'عناصر الحياة' : 'Elements of Life'}
              <div className="absolute -bottom-1 left-0 w-0 h-px bg-primary transition-all duration-300 group-hover:w-full"></div>
            </Link>

            <Link to="/biogeometry" className="relative group text-sm tracking-wide">
            ...
            >
              <button className="flex items-center gap-1 text-sm tracking-wide hover:text-primary transition-colors">
                ...
                <ChevronDown className="w-4 h-4 transition-transform" style={{ transform: digitalBooksMenu ? 'rotate(180deg)' : 'rotate(0)' }} />
              </button>
              <AnimatePresence>
                {digitalBooksMenu && (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="px-6 py-4 border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
                      <h3 className="text-base font-medium" style={{ fontFamily: 'var(--font-heading)' }}>
                        {language === 'ar' ? 'الكتب الرقمية' : 'Digital Books'}
                      </h3>
                      <p className="text-xs text-muted-foreground mt-1">
                        {language === 'ar' ? 'تنزيل فوري، وصول مدى الحياة' : 'Instant download, lifetime access'}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 p-6 gap-6">
                      ...
                      <div className="space-y-3 border-l border-border/50 pl-6">
                        <h4 className="text-xs uppercase text-muted-foreground mb-3">{language === 'ar' ? 'روابط سريعة' : 'Quick Links'}</h4>
                        <Link to="/shop/digital" className="flex items-center gap-2 py-2.5 px-3 rounded-lg hover:bg-muted/50 text-sm hover:text-primary transition-all group">
                          <ChevronDown className="w-4 h-4 -rotate-90 group-hover:translate-x-1 transition-transform" />
                          {language === 'ar' ? 'جميع الكتب الرقمية' : 'All Digital Books'}
                        </Link>
                        <Link to="/shop" className="flex items-center gap-2 py-2.5 px-3 rounded-lg hover:bg-muted/50 text-sm hover:text-primary transition-all group">
                          <ChevronDown className="w-4 h-4 -rotate-90 group-hover:translate-x-1 transition-transform" />
            ...
            >
              <button className="flex items-center gap-1 text-sm tracking-wide hover:text-primary transition-colors">
                ...
                <ChevronDown className="w-4 h-4 transition-transform" style={{ transform: printedBooksMenu ? 'rotate(180deg)' : 'rotate(0)' }} />
              </button>
              <AnimatePresence>
                {printedBooksMenu && (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="px-6 py-4 border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {language === 'ar' ? 'جودة ممتازة، شحن سريع' : 'Premium quality, fast shipping'}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 p-6 gap-6">
                      ...
                      <div className="space-y-3 border-l border-border/50 pl-6">
                        <h4 className="text-xs uppercase text-muted-foreground mb-3">{language === 'ar' ? 'روابط سريعة' : 'Quick Links'}</h4>
                        <Link to="/shop/physical" className="flex items-center gap-2 py-2.5 px-3 rounded-lg hover:bg-muted/50 text-sm hover:text-primary transition-all group">
                          <ChevronDown className="w-4 h-4 -rotate-90 group-hover:translate-x-1 transition-transform" />
                          {language === 'ar' ? 'جميع الكتب المطبوعة' : 'All Printed Books'}
                        </Link>
                        <Link to="/shop" className="flex items-center gap-2 py-2.5 px-3 rounded-lg hover:bg-muted/50 text-sm hover:text-primary transition-all group">
                          <ChevronDown className="w-4 h-4 -rotate-90 group-hover:translate-x-1 transition-transform" />
                          {language === 'ar' ? 'تصفح جميع المنتجات' : 'Browse All Products'}
            ...
            <>
              ...
              >
                ...
                <div className="flex items-center justify-between p-4 sm:p-5 border-b border-border bg-gradient-to-r from-primary/5 to-transparent shrink-0">
                  <img src={language === 'ar' ? logoLunologyAr : logoLunologyEn} alt="Lunology" className="h-10 sm:h-12 w-auto" />
                  <button
                    onClick={() => setIsOpen(false)}
                ...
                {/* Mobile Menu Content */}
                <div className="flex-1 overflow-y-auto overscroll-contain py-6 px-4" style={{ minHeight: 0 }}>
                  <nav className="space-y-2">
                    ...
                    <div className="pt-4 pb-2">
                      <h3 className="px-4 text-xs uppercase text-muted-foreground mb-3">
                        {language === 'ar' ? 'الكتب الرقمية' : 'Digital Books'}
                      </h3>
                      <Link
                        to="/shop/digital"
                    ...
                    <div className="pt-4 pb-2">
                      ...
                      {printedProducts.slice(0, 2).map((product) => (
                        <Link
                          key={product.id}
                          to={`/product/${product.id}`}
                          className="flex items-center gap-3 py-2.5 px-4 rounded-lg hover:bg-muted transition-colors"
